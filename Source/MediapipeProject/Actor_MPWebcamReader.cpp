...
#include "Actor_MPWebcamReader.h"
...
// Sets default values
AActor_MPWebcamReader::AActor_MPWebcamReader()
{
    ...
    // TODO:: set the Real Would camera initialization
    CameraID = 1;
    RefreshRate = 30;
    isStreamOpen = false;
...
void AActor_MPWebcamReader::BeginPlay()
{
	Super::BeginPlay();

#ifdef WEBCAMREADER
    // Open the stream
    // TODO:: Open the Real Would camera 
...
// Called every frame
void AActor_MPWebcamReader::Tick(float DeltaTime)
{
...
#ifdef WEBCAMREADER
void AActor_MPWebcamReader::UpdateFrame()
{
...
void AActor_MPWebcamReader::DoProcessing()
{
        ...
        if (p_FuncMPPSendPacket != NULL)
        {
            ...
            for (int i = 0; i < HAND_LANDMARK_NUM; i++) {
                ...
                if (!strcmp(mp_holistic->HandsLabel[LEFT_HAND], "Left")) {
                    ...
                    else { CoordnateArray[LEFT_HAND][i][0] = VideoSize.X + 10; }
...
void AActor_MPWebcamReader::UpdateTexture()
{
...
bool AActor_MPWebcamReader::HasPoseLandmarkCoords()
{
...
FVector AActor_MPWebcamReader::GetHandsLandmarkCoords(int HandIndex, int LandmarkIndex)
{
    if(mp_holistic != nullptr)
    {
        ...
        if(HandIndex == LEFT_HAND && strcmp(mp_holistic->HandsLabel[LEFT_HAND], "Left"))
            return FVector::ZeroVector;
        const float X = mp_holistic->HandsLandmarkCoords[HandIndex][LandmarkIndex][0];
        const float Y = mp_holistic->HandsLandmarkCoords[HandIndex][LandmarkIndex][1];
...
FVector AActor_MPWebcamReader::GetPoseLandmarkCoords(int LandmarkIndex)
{
    if(mp_holistic != nullptr && !strcmp(mp_holistic->PoseLabel, "Pose"))
    {
...
void AActor_MPWebcamReader::UpdateTextureRegions(UTexture2D* Texture, int32 MipIndex, uint32 NumRegions, FUpdateTextureRegion2D* Regions, uint32 SrcPitch, uint32 SrcBpp, uint8* SrcData, bool bFreeData)
{
